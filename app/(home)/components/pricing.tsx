...
import { PricingCart } from '.'

export const Pricing = () => {
  return (
    <div className='max-w-7xl mx-auto container text-center flex justify-center flex-col items-center'>
        <h1 className='text-2xl sm:text-3xl md:text-4xl font-bold max-w-2xl text-center'>Kompaniyangiz uchun yagona tizim. Jamoaviy sinab ko'ring</h1>
        <p className='uppercase opacity-70 mt-4'>Quyidagi jamoalar tomonidan ishonilgan:</p>
        <div className='flex gap-4 flex-row flex-wrap mt-4'>
            {teams.map((team, index) => (
                <Image src={team} alt={'team'} key={index} width={50} height={50}  />
            ) )}
        </div>
        <div className="mt-6">
            <div className="space-y-8 lg:grid lg:grid-cols-3 sm:gap-6 xl:gap-10 lg:space-y-0">
                {cards.map((card, index) => (
                    <PricingCart key={index} {...card} /> 
                ))}
            </div>
        </div>
    </div>
